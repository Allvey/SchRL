...
from parl.algorithms.paddle import DQN, MDQN
import visdom
import time
    ...
    while True:
        ...
        if (len(rpm) > MEMORY_WARMUP_SIZE) and (step % LEARN_FREQ == 0):
            ...
            train_loss = agent.learn(batch_obs, batch_action, batch_reward,
                                     batch_next_obs, batch_done, np.ones((BATCH_SIZE, 1)))
...
def main():
    ...
    # alg = PrioritizedDoubleDQN(model, act_dim=act_dim, gamma=GAMMA, lr=LEARNING_RATE)
    agent = DispatchAgent(
